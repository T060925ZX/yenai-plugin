...
async function verifyByLink(userId, groupId, e) {
  ...
  try {
    ...
    try {
      ...
      logger.warn(`${Log_Prefix}[授权链接验证]API可能已变更或不可用，降级到数学计算验证`)
    ...
    let currentLoginCode = login_code
    ...
    const pollTimer = setInterval(async () => {
      ...
      if (pollCount > 0 && pollCount % 60 === 0) {
        ...
        try {
          ...
          if (newResult.data && newResult.data.code) {
            currentLoginCode = newResult.data.code
            const newAuthUrl = `https://h5.qzone.qq.COM/qqq/code/${currentLoginCode}?_proxy=1&from=ide`
      ...
      // 检查授权状态
      try {
        let checkResponse = await fetch(`https://q.qq.com/ide/devtoolAuth/syncScanSateGetTicket?code=${currentLoginCode}`, options)
